...
def hanoi_solver(n):
    ...
    def record_state():
        line = f"{rods['A']} {rods['B']} {rods['C']}"
        moves.append(line)

    def move_disks(count, source, auxiliary, target):
        if count == 0:
            return
        # Step 1: move the top (count - 1) disks out of the way, onto auxiliary
        move_disks(count - 1, source, target, auxiliary)

        # Step 2: move the single remaining (largest of this batch) disk to target
        disk = rods[source].pop()
        rods[target].append(disk)
        record_state()

        # Step 3: move the (count - 1) disks from auxiliary onto target
        move_disks(count - 1, auxiliary, source, target)

    # Record the starting arrangement first
    record_state()

    # Solve: move all n disks from rod A to rod C, using rod B as auxiliary
    move_disks(n, 'A', 'B', 'C')
