...
class HashTable:
    ...
    def add(self, key, value):
        # Step 1: find the hash for this key
        hash_value = self.hash(key)

        # Step 2: if this hash isn't in the collection yet, create an empty dict for it
        if hash_value not in self.collection:
            self.collection[hash_value] = {}

        # Step 3: store the key-value pair inside that dict
        self.collection[hash_value][key] = value

    def remove(self, key):
        hash_value = self.hash(key)

        # only try to remove if the hash exists AND the key exists inside it
