...
class an __init__ method with the following parameters: self, title, year, director,

and duration.Inside
the
__init__
method, assign
title, year, director, and duration
parameters
to
instance
attributes
...
readable
display
for debugging and printing.Create a __str__ method that returns a string with the format title (year) - duration min, director (where title, year, duration, and director should be replaced with the values of the corresponding attributes) so that printing a movie gives a nice summary.
...
movie1 = Movie("Hitler", "2026", "Mr Rob", "2 hours")
print(movie1)
...
movie
object.Now, you
'll add some validation to make sure that movies cannot be instantiated with invalid attributes.
...
that
checks if year is less
than
1895 and
raise a
ValueError
with the message Year must be 1895 or later.
...
another if statement
to
check if director is either
empty or contains
whitespace
only and
raise a
ValueError
with the message Director cannot be empty.
...
last if statement
to
verify if duration is not greater
than
0 and
raise a
ValueError
with the message Duration must be positive.
...
movie1
assignment in a
try block and create an except clause that catches ValueError as e and uses an f-string to print Validation Error: followed
by
a
space and the
error
...
class named MediaCatalogue, and give it an __init__ method that sets an items attribute to an empty list.This will represent the list of items in the catalogue.
...
class needs a method to add media items to the catalogue.Create an add method that accepts an item media_item and appends it to the items list.
...
11
You
'll need a proper way to visually represent the catalogue. So create a __str__ method in your MediaCatalogue class.
...
an
f - string
with the format Media Catalogue (N items):.
Replace
N
with the number of items in the items list and make the string end with two newline characters.
...
variable, create
a
for loop that uses enumeration to iterate over the items in the catalogue and appends to result a string with the following format: N.movie
where
N is a
numeric
index
...
with a newline character and return result after the loop.

ANS:
result = f'Media Catalogue ({len(self.items)} items):\n\n'
for index, movie in enumerate(self.items, 1):
...
Before
the
try block, create an instance of MediaCatalogue and assign it to a variable named catalogue.
...
object
instantiated
with arguments of your choice.Then append movie2 to the catalogue.
...
ANS:
    ...
    series2 = TVSeries('Avatar', 2026, 'Beethoven', 32, 5, 300)
...
ANS:
    catalogue.add("series2")  #Media Error: Only Movie or TVSeries instances can be added
...
ANS:
    ...
    print(f'Media Error: {e}')
    print(f'Unable to add {e.obj}: {type(e.obj)}')  #Unable to add series2: <class 'str'>
